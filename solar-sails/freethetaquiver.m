...
figure
plot(xl(:,1).*cos(xl(:,2)),xl(:,1).*sin(xl(:,2)),'LineWidth',5)
hold on
yline(0);
xline(0);
plot(0,0,'o')
[x, y] = pol2cart(0:0.01:2*pi, ones(1,length(0:0.01:2*pi)));
[x2, y2] = pol2cart(0:0.01:2*pi, r_f*ones(1,length(0:0.01:2*pi)));
plot(x,y)
plot(x2,y2)
axis equal;
...
quiver(R.*cos(T),R.*sin(T),cos(ALP),sin(ALP))
...
% plot(t*(58.13/365.25),alp*180/pi,'LineWidth',2,'LineStyle','--')
